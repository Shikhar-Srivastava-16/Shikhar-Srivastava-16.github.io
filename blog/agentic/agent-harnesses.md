...
Luckily, the fix for this is relatively simple. Let us look at our problems and their possible solutions:
 ...
 1. **Interpreted languages are slow**: We solve this just move our platform to a compiled language, preferably one which is fast, has a strong type system and an optimised compiler. Compiled languages go a significant way to solving these problems by moving much of the processing to compile-time, and far less computing is required at runtime. For example, the compiler can optimise away several function calls using a process called *inlining. Another thing to keep in mind when selecting our new language is that different languages require different amounts dynamic computing. This is because the structure of a languages determines what types of computing are Turing-Decidable. All undecidable computing is done at runtime.
